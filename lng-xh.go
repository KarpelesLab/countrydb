package countrydb

var Localexh = map[string]Translated{
	"AF": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"AO": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"AX": Translated{
		Name: "Ireland",
	},
	"AR": Translated{
		Name:         "Argentina",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"AM": Translated{
		Name: "Argentina",
	},
	"AS": Translated{
		Name: "Melika, Mntla",
	},
	"AG": Translated{
		Name: "Antigua no Barbuda",
	},
	"AU": Translated{
		Name: "Australiya",
	},
	"AT": Translated{
		Name: "Austria",
	},
	"AZ": Translated{
		Name:         "Azerbaijan",
		OfficialName: "Azerbaijan",
	},
	"BE": Translated{
		Name: "Belgium",
	},
	"BJ": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"BD": Translated{
		Name: "Bangladesh",
	},
	"BG": Translated{
		Name: "Bulgaria",
	},
	"BH": Translated{
		Name:         "Brazil",
		OfficialName: "Brazil",
	},
	"BS": Translated{
		Name: "Panama",
	},
	"BY": Translated{
		Name: "Belarus",
	},
	"BZ": Translated{
		Name: "Belgium",
	},
	"BO": Translated{
		CommonName: "Bolivia",
	},
	"BR": Translated{
		Name: "Brazil",
	},
	"BB": Translated{
		Name: "Barbados",
	},
	"CF": Translated{
		Name: "Ulawulo lokuziphatha kwe Dominican",
	},
	"CA": Translated{
		Name: "Canada",
	},
	"CH": Translated{
		Name: "Switzerland",
	},
	"CL": Translated{
		Name:         "Chile",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"CN": Translated{
		Name:         "Tshayina",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"CI": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"CO": Translated{
		Name: "Columbia",
	},
	"CV": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"CU": Translated{
		Name:         "Columbia",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"CY": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"CZ": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"DE": Translated{
		Name:         "Jamani",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"DM": Translated{
		Name: "Romania",
	},
	"DK": Translated{
		Name: "Denmark",
	},
	"DO": Translated{
		Name: "Ulawulo lokuziphatha kwe Dominican",
	},
	"DZ": Translated{
		Name: "Bulgaria",
	},
	"EC": Translated{
		Name: "Equador",
	},
	"EG": Translated{
		Name:         "Amandla",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"ER": Translated{
		OfficialName: "United States ye Melika",
	},
	"ES": Translated{
		Name: "Spain",
	},
	"EE": Translated{
		Name: "Estonia",
	},
	"ET": Translated{
		Name: "Estonia",
	},
	"FI": Translated{
		Name: "Finland",
	},
	"FJ": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"FR": Translated{
		Name:         "Franisi",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"FO": Translated{
		Name: "Ireland",
	},
	"GA": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"GE": Translated{
		Name: "Serbia",
	},
	"GH": Translated{
		Name:         "Tshayina",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"GN": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"GM": Translated{
		Name:         "Jamaica",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"GR": Translated{
		Name:         "Greece",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"GD": Translated{
		Name: "Grenada",
	},
	"GL": Translated{
		Name: "Grenada",
	},
	"GT": Translated{
		Name:         "Guatemala",
		OfficialName: "Guatemala",
	},
	"GU": Translated{
		Name: "Guatemala",
	},
	"GY": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"HN": Translated{
		Name: "Honduras",
	},
	"HR": Translated{
		Name: "Croatia",
	},
	"HT": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"HU": Translated{
		Name:         "Hungary",
		OfficialName: "Hungary",
	},
	"ID": Translated{
		Name:         "Indonesia",
		OfficialName: "Indonesia",
	},
	"IN": Translated{
		Name:         "Indonesia",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"IE": Translated{
		Name: "Ireland",
	},
	"IR": Translated{
		Name:         "Ulawulo lokuziphatha kwe Dominican",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"IQ": Translated{
		Name:         "Sirayeli",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"IS": Translated{
		Name: "Iceland",
	},
	"IL": Translated{
		Name: "Sirayeli",
	},
	"IT": Translated{
		Name:         "Ithali",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"JM": Translated{
		Name: "Jamaica",
	},
	"JO": Translated{
		Name: "isiKorea",
	},
	"JP": Translated{
		Name: "Japhani",
	},
	"KZ": Translated{
		Name:         "Taiwan",
		OfficialName: "Taiwan",
	},
	"KE": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"KG": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"KH": Translated{
		Name: "Columbia",
	},
	"KN": Translated{
		Name: "St. Kitts no Nevis",
	},
	"KR": Translated{
		Name:       "Ulawulo lokuziphatha kwe Dominican",
		CommonName: "Mzantsi Afrika",
	},
	"LB": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"LR": Translated{
		Name: "Serbia",
	},
	"LY": Translated{
		Name:         "Serbia",
		OfficialName: "Serbia",
	},
	"LC": Translated{
		Name: "St. Lucia",
	},
	"LT": Translated{
		Name:         "Lithuania",
		OfficialName: "Lithuania",
	},
	"LU": Translated{
		Name: "Luxembourg",
	},
	"LV": Translated{
		Name:         "Latvia",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"MO": Translated{
		Name: "Malta",
	},
	"MD": Translated{
		Name: "Ulawulo lokuziphatha kwe Dominican",
	},
	"MV": Translated{
		Name: "Malta",
	},
	"MX": Translated{
		Name:         "Mexico",
		OfficialName: "United States ye Melika",
	},
	"MK": Translated{
		Name:         "Macedonia",
		OfficialName: "Lithuania",
	},
	"ML": Translated{
		Name:         "Malta",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"MT": Translated{
		Name:         "Malta",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"MM": Translated{
		Name:         "Panama",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"MR": Translated{
		Name: "Lithuania",
	},
	"MU": Translated{
		Name:         "Lithuania",
		OfficialName: "Lithuania",
	},
	"MW": Translated{
		Name:         "Malta",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"MY": Translated{
		Name: "Malta",
	},
	"NC": Translated{
		Name: "Macedonia",
	},
	"NG": Translated{
		Name:         "Serbia",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"NI": Translated{
		Name:         "Paraguay",
		OfficialName: "Paraguay",
	},
	"NL": Translated{
		Name: "Netherlands",
	},
	"NO": Translated{
		Name: "Norway",
	},
	"NP": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"NR": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"NZ": Translated{
		Name: "New Zealand (izealand entsha)",
	},
	"PA": Translated{
		Name:         "Panama",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"PE": Translated{
		Name:         "Peru",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"PW": Translated{
		Name:         "Paraguay",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"PL": Translated{
		Name:         "Poland",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"KP": Translated{
		CommonName: "Macedonia",
	},
	"PT": Translated{
		Name:         "Portugal",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"PY": Translated{
		Name: "Paraguay",
	},
	"PS": Translated{
		OfficialName: "United States ye Melika",
	},
	"QA": Translated{
		Name: "Malta",
	},
	"RO": Translated{
		Name: "Romania",
	},
	"RW": Translated{
		Name:         "Canada",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"SD": Translated{
		Name: "Isandi",
	},
	"SV": Translated{
		Name:         "El Salvador",
		OfficialName: "El Salvador",
	},
	"SO": Translated{
		Name:         "Romania",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"RS": Translated{
		Name:         "Serbia",
		OfficialName: "Azerbaijan",
	},
	"SS": Translated{
		Name:         "Mzantsi Afrika",
		OfficialName: "Mzantsi Afrika",
	},
	"SK": Translated{
		Name:         "Slovakia",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"SI": Translated{
		Name: "Slovenia",
	},
	"SE": Translated{
		Name: "Sweden",
	},
	"SZ": Translated{
		OfficialName: "Thailand",
	},
	"SY": Translated{
		Name: "Ulawulo lokuziphatha kwe Dominican",
	},
	"TD": Translated{
		Name:         "Tshayina",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"TG": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"TH": Translated{
		Name: "Thailand",
	},
	"TJ": Translated{
		Name:         "Taiwan",
		OfficialName: "Taiwan",
	},
	"TT": Translated{
		Name:         "Trinidad ne Tobago",
		OfficialName: "Trinidad ne Tobago",
	},
	"TN": Translated{
		Name: "Rashiya",
	},
	"TR": Translated{
		Name:         "Turkey",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"TW": Translated{
		CommonName: "Taiwan",
	},
	"TZ": Translated{
		Name: "Ulawulo lokuziphatha kwe Dominican",
	},
	"UG": Translated{
		Name:         "Canada",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"UA": Translated{
		Name: "Ukraine",
	},
	"UM": Translated{
		Name: "United States ye Melika",
	},
	"UY": Translated{
		Name: "Uruguay",
	},
	"US": Translated{
		Name:         "United States ye Melika",
		OfficialName: "United States ye Melika",
	},
	"VC": Translated{
		Name: "St. Vincent ne Grenadines",
	},
	"VE": Translated{
		Name:         "Ulawulo lokuziphatha kwe Dominican",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
		CommonName:   "Venezuela",
	},
	"VG": Translated{
		OfficialName: "Finland",
	},
	"VI": Translated{
		Name:         "Finland",
		OfficialName: "Finland",
	},
	"VN": Translated{
		Name:       "Vietnam",
		CommonName: "Vietnam",
	},
	"YE": Translated{
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
	"ZA": Translated{
		Name:         "Mzantsi Afrika",
		OfficialName: "Mzantsi Afrika",
	},
	"ZM": Translated{
		Name:         "Jamaica",
		OfficialName: "Ulawulo lokuziphatha kwe Dominican",
	},
}
